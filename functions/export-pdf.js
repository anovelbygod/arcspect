...
function buildPDF(markdown) {
  return new Promise((resolve, reject) => {
    ...
    const L = 56;
    const pageW = doc.page.width - L - 56;
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith('# ')) {
        ensureSpace(72);
        doc.y += 8;
        doc.font('Inter-Bold').fontSize(22).fillColor(C.text).text(line.slice(2).trim(), L, doc.y, { width: pageW });
        doc.y += 4; hRule(C.green, 2); doc.y += 6;
        i++; continue;
      }
      ...
      if (line.startsWith('## ') && !line.startsWith('### ')) {
        ...
        doc.y += 5; hRule(C.ruleThin, 0.5); doc.y += 2;
        i++; continue;
      }
      ...
      if (line.startsWith('### ')) {
        ensureSpace(32);
        doc.y += 8;
        doc.font('Inter-Bold').fontSize(12).fillColor(C.text).text(line.slice(4).trim(), L, doc.y, { width: pageW });
        doc.y += 4;
        i++; continue;
      }
      ...
      if (line.trim().startsWith('|')) {
        ...
        headers.forEach((h, ci) => {
          ...
          doc.font('Inter-Bold').fontSize(8).fillColor(C.green)
             .text(h.toUpperCase(), x + 8, sy + 8, { width: colW - 16, ellipsis: true });
        });
        doc.y = sy + hRowH;
      ...
      if (/^- \[[ x]\] /.test(line)) {
        ...
        doc.font('Inter').fillColor(C.text2).text('  ' + cleanInline(line.slice(6).trim()), { width: pageW - 18 });
        doc.y += 2; i++; continue;
      }
      ...
      if (/^- /.test(line)) {
        ...
        doc.font('Inter').fontSize(11).fillColor(C.text2)
           .text(' ' + cleanInline(line.slice(2).trim()), { width: pageW - 16 });
        doc.y += 2; i++; continue;
      }
      ...
      if (/^\d+\. /.test(line)) {
        ...
        doc.font('Inter-Bold').fontSize(11).fillColor(C.muted)
           .text(num + '.', L, startY, { width: numW });
        doc.font('Inter').fontSize(11).fillColor(C.text2)
           .text(text, L + numW + 4, startY, { width: pageW - numW - 4 });
        doc.y += 4; i++; continue;
      }
      ...
      if (boldMatch) {
        doc.font('Inter-Bold').fontSize(11).fillColor(C.text)
           .text(boldMatch[1] + ': ', L, doc.y, { continued: true, width: pageW });
        doc.font('Inter').fillColor(C.text2).text(cleanInline(boldMatch[2]));
      } else {
        doc.font('Inter').fontSize(11).fillColor(C.text2)
